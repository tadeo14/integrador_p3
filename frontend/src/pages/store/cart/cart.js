  ...
  `;
    container.querySelectorAll('button[data-action]').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (action === 'del')
                removeFromCart(id);
            else if (action === 'inc' && item.cantidad < item.stock)
...
function fillOrderSummary(cart, subtotal, total) {
...
document.getElementById('closeModal').addEventListener('click', () => document.getElementById('checkoutModal').classList.remove('open'));
document.getElementById('checkoutForm').addEventListener('submit', (e) => {
    ...
    const pedidos = JSON.parse(localStorage.getItem('foodstore_pedidos') || '[]');
    pedidos.push(pedido);
