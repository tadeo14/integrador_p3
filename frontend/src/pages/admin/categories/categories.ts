...
function renderTable() {
  ...
  body.querySelectorAll('button[data-action]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      else if (action === 'del') deleteCategoria(id)
    })
  })
}
...
function openModal(id?: number) {
  ...
  ;(document.getElementById('editId')      as HTMLInputElement).value = cat ? String(cat.id) : ''
  ;(document.getElementById('nombre')      as HTMLInputElement).value = cat?.nombre ?? ''
  ;(document.getElementById('descripcion') as HTMLInputElement).value = cat?.descripcion ?? ''
  ;(document.getElementById('imagen')      as HTMLInputElement).value = cat?.imagen ?? ''
