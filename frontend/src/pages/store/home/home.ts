...
let selectedCatId: number | null = null
...
async function init() {
  ...
  catList.querySelector('[data-id="all"]')!.addEventListener('click', () => {
    selectedCatId = null
    document.querySelectorAll('.cat-sidebar li').forEach(el => el.classList.remove('active'))
    ;(catList.querySelector('[data-id="all"]') as HTMLElement).classList.add('active')
    renderGrid()
  })
...
function renderGrid() {
  ...
  if (sort === 'nombre-asc')  productos.sort((a, b) => a.nombre.localeCompare(b.nombre))
  if (sort === 'nombre-desc') productos.sort((a, b) => b.nombre.localeCompare(a.nombre))
  if (sort === 'precio-asc')  productos.sort((a, b) => a.precio - b.precio)
  if (sort === 'precio-desc') productos.sort((a, b) => b.precio - a.precio)
