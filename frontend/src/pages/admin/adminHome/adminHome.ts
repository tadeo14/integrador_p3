...
async function init() {
  ...
  const byEstado = ['PENDIENTE','CONFIRMADO','TERMINADO','CANCELADO'].map(e =>
    `<span class="badge ${e==='PENDIENTE'?'badge-yellow':e==='CONFIRMADO'?'badge-blue':e==='TERMINADO'?'badge-green':'badge-red'}">${e}: ${pedidos.filter(p=>p.estado===e).length}</span>`
  ).join(' ')
